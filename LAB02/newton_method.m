...
% List of Newton's method variants and corresponding gamma computation methods
method = ["newton_const", "newton_gamma", "newton_armijo"];
gamma_method = ["const", "minimized with modified golden section", "armijo"];

% Set optimization parameters
epsilon = 0.01; % Convergence criterion based on gradient norm
gamma = 0.5;    % Initial step size (used for "const" gamma method)
figno = 1;      % Initialize figure numbering

% Iterate over starting points and Newton's method variants
for i = 1:3  % Loop through the three starting points
    for j = 1:3  % Loop through the three Newton's method variants
        % Dynamically call the appropriate Newton's method function
